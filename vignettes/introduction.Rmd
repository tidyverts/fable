...
Suppose we wanted to forecast the number of people travel domestically to Melbourne, Australia. In the `tsibble::tourism` dataset, this can be further broken down into 4 reasons of travel: "business", "holiday", "visiting friends and relatives" and "other reasons". The first observation from each series is shown below.
...
The `glance()` function provides a one-row summary of each model, and commonly includes descriptions of the model's fit such as the residual variance and information criterions. Be wary though, as information criterions (AIC, AICc, BIC) are only comparable between the same model class and only if those models share the same response (after transformations and differencing).
...
Forecasts from these models can be produced directly as our specified models do not require on any additional data.
...
The resulting forecasts are contained in a "fable" (forecast table), and both point forecasts (`Trips`) and forecast distributions (`.distribution`) are available in the table for the next five years.
...
It may be more convenient to use `hilo()` on the entire fable, which allows you to extract multiple intervals at once.
...
Lastly you can see a plot of the forecasts using `autoplot()`. To see the historical data along with the forecasts you can provide it as the first argument to the function.
...
More information about modelling time series and using the fable package can be found in [*Forecasting: Principles and Practices (3rd Ed.)*](https://otexts.com/fpp3/) and in the [*pkgdown site*](fable.tidyverts.org/).